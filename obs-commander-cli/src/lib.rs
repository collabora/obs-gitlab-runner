...
use obs_commander::{
    ...
    artifacts::{ArtifactDirectory, MissingArtifact, ScopedCell},
    build_meta::RepoArch,
...
pub enum CliAction {
    ...
    Monitor {
        ...
        #[clap(long, hide = true, env = "OBS_COMMANDER_TEST_LOG_TAIL", default_value_t = LOG_TAIL_2MB)]
        log_tail: u64,
        #[clap(long, hide = true, env = "OBS_COMMANDER_TEST_SLEEP_ON_BUILDING_MS")]
        sleep_on_building_ms: Option<u64>,
        #[clap(long, hide = true, env = "OBS_COMMANDER_TEST_SLEEP_ON_OLD_STATUS_MS")]
...
impl ArtifactDirectory for LocalFsArtifacts {
    async fn open(&self, path: impl AsRef<Utf8Path> + Send) -> Result<AsyncFile> {
        ...
        AsyncFile::open(&path).await.map_err(|e| {
        ...
        Report: From<Err>,
    {
        ...
        let file = AsyncFile::from_std(temp.as_file().try_clone()?);
...
impl Handler {
    ...
    async fn generate_monitor(&mut self, args: GenerateMonitorAction) -> Result<()> {
        ...
        self.artifacts
            ...
            .await?;
        info!("Wrote monitor file '{}'.", args.monitor_out);
