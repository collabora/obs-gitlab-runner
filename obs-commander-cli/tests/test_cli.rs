...
use camino::Utf8Path;
use claims::*;
use obs_commander::actions::ObsBuildInfo;
use obs_commander_cli::{DEFAULT_MONITOR_TABLE, MonitorTable};
...
impl RunBuilder<'_> for CliRunBuilder {
    ...
    fn script(mut self, script: &[String]) -> Self {
        for line in script {
            self.script
                .push(format!("{} {line}", env!("CARGO_BIN_EXE_obs-commander")));
        }
        self
    }
    ...
    async fn go(self) -> Self::ExecutionResult {
        ...
        let mut child = Command::new("sh")
            ...
            .env(
                "OBS_COMMANDER_TEST_LOG_TAIL",
                MONITOR_TEST_LOG_TAIL.to_string(),
            )
            .env("OBS_COMMANDER_TEST_SLEEP_ON_BUILDING_MS", "0")
            .env(
                "OBS_COMMANDER_TEST_SLEEP_ON_OLD_STATUS_MS",
        ...
        let mut output = LinesStream::new(stdout.lines()).merge(LinesStream::new(stderr.lines()));
        let mut lines = vec![];
