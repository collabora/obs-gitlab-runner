...
impl Actions {
    ...
    pub async fn dput(
        ...
        if result.unchanged {
            info!("Package unchanged at revision {}.", result.rev);
            ...
            info!("Package uploaded with revision {}.", result.rev);
        }
    ...
    pub async fn monitor<F: Future<Output = Result<()>> + Send>(
        ...
        log_tail_cb: impl FnOnce(AsyncFile) -> F,
        log_tail_bytes: u64,
        artifacts: &mut impl ArtifactDirectory,
    ) -> Result<()> {
        ...
        match completion {
            ...
            PackageCompletion::Superceded => {
                info!("Build was superceded by a newer revision.");
            }
            PackageCompletion::Disabled => {
                info!("Package is disabled for this architecture.");
            }
            PackageCompletion::Failed(reason) => {
                ...
                info!("The last 2MB of the build log is printed above.");
                info!(
    ...
    pub async fn download_binaries(
        ...
        info!("Downloaded {} artifact(s).", binaries.paths.len());
        Ok(())
    }
    ...
    pub async fn prune(
        ...
        if build_info.is_branched {
            info!(
                ...
                build_info.project, build_info.package
            );
            prune_branch(
                &self.client,
                &build_info.project,
            ...
            info!("Skipping prune: package was not branched.");
        }
